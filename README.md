...
**Secrets note.** Launchd jobs run with a restricted environment. If your
`config.toml` uses `_cmd` fields that depend on tools requiring a
terminal/keychain unlock (e.g. `op`, `security`), the watchdog may fail
to authenticate. The most reliable options for an unattended agent are:
